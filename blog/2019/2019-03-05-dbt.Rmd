...
In addition, we can define different target schemas (for BigQuery this equals *datasets*) where tables and views are going to be materialized. It is recommended to have at least one dev environment besides the production tables for experimentation and prototyping.
...
Models can be run from the command line using `dbt compile` and `dbt run`. The former compiles all the jinja magic into actual SQL queries. The incremental models are for example expanded into a combination of merge, insert and delete statements depending on the backend. The `dbt run` command runs all enabled models and materializes the results. [Additional flags](https://docs.getdbt.com/reference#run) can be supplied to select or exclude specific models from running. 
...
```{sql}
{% macr rolling_avg(value_col, time_col, nr_rows) %}
...
One of my favorite features of dbt is its testing framework. We can run two kind of tests, schema tests and data tests. There are [schema validation tests](https://docs.getdbt.com/docs/testing) included out of the box for uniqueness, missing values, relationships or accepted values. 

These tests are effectively macros, which we could just as well write ourselves. It is in fact possible to create a dbt package consisting of macros only. In this sense dbt encourages the development of utility packages that can be shared and reused. A good example is [dbt-utils](https://github.com/fishtown-analytics/dbt-utils), that among others contains some more tests. To use external packages, we need an extra `packages.yml` file in the package root directory to list dependencies.
...
In each case schema tests need to be declared inside `schema.yml` in the `models/` directory. 
...
While there is no native scheduling mechanism, dbt does provide a managed service called [dbt Cloud that you can sign up for](https://cloud.getdbt.com/). Besides scheduling jobs, you can view the logs of completed runs or render the documentation. The service is completely free of charge and is your best bet if you intend to use the dbt in production. 
