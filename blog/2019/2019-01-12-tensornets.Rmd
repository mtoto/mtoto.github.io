...
It's been at least a year since I last dabbled in TensorFlow, and one year is helluva long time, or to say the ecosystem has been evolving at furious speed. The general trend has been towards higher-level abstractions, be it building models, defining data pipelines or serving predictions. Then there is the relentless community-driven development, spawning many useful extensions to the official library. Yet, using TensorFlow still has a bit of going down into the trenches feel to it compared to fitting a model in sci-kit learn. 

What started as casual browsing of the official documentation, lead to a strong urge to try out some of the new tools. In particular, I was eager to implement data ingestion and transformation using the relatively new [Dataset API](https://www.tensorflow.org/guide/datasets). And secondly, I was looking for an easy way to do *Transfer Learning*, and found the [tensornets](https://github.com/taehoonlee/tensornets) library which has many available models and a seemingly easy-to-use syntax. 
...
There is a wide range of pre-trained model to choose from for object detection. Thanks to extensive research in the area performance have been skyrocketing. The latest models can detect 1,000 different objects in a regular picture with 95% accuracy. 
...
with tf.control_dependencies(update_ops):               # only train last "block"
    train = tf.train.AdamOptimizer(1e-5).minimize(loss, var_list[520:]) 
 
with tf.Session() as sess:
    sess.run(model.pretrained())
     
    for epoch in range(10):
       for (x, y) in your_data:  
           # run training
           sess.run(train, feed_dict={inputs: x, outputs: y, is_train: True})
       # run testing           
       sess.run(accuracy, feed_dict={inputs: x, outputs: y, is_train: False}) # use global stats for batch norm
```


## Preprocessing with the Dataset API

A common way to feed data in TensorFlow is with `feed_dict`. Training data is usually loaded as numpy arrays and reshaped to `(batch_size*pixel*pixel*channel)`, which is the shape of the placeholder. If your data doesn't fit into memory, you are out of luck and will be forced to write *a lot* of boilerplate code for I/O. Besides the verbosity, this approach tends to be slower as computation ends up waiting for new data.
...
For the purpose of this post, I took on the fer2013 dataset from the Kaggle competition [Facial Expression Recognition Challenge](https://www.kaggle.com/c/challenges-in-representation-learning-facial-expression-recognition-challenge/data) from six years ago. The training data consists of 28.709 grayscale images with 48*48 pixel resolution. The target column is an integer indicating one of the 7 emotions, and the pixels column is a space separated string of 2.304 pixels. A rather unusual format for image data if you ask me. To get an idea though, here is three pictures from the dataset:

![](fer1.png "title-1") ![](fer2.png "Angry")


With the Dataset API, we can load data from many sources including numpy arrays, tensors, text files, csv's or even folders. In addition, dataset objects come with a handful handy methods for data preparation. 
...
The `parse_csv()` defines how get from the aforementioned .csv file to tensor. This can be written using TensorFlow functions too. 
...
Running `sess.run(training_init_op)` before training and `sess.run(validation_init_op)` before testing ensures that we are loading the right dataset. Defining data preparation this way, we can get rid of the `feed_dict` mechanism for data ingestion and can plug the values from `iterator.get_next()` straight into the graph instead. Last but not least, we should also checkpoint the model's progress `tf.train.Saver()`.
...
with tf.Session() as sess:
       ...
       for epoch in range(hparams.num_epochs):
             ...
             for _ in range(224): # 7178 / 32
                 ...
                 avg_acc += acc
             print("Average validation set accuracy is {:.2f}%".format(avg_acc / 224 * 100))
             saver.save(sess, "gs://path/to/ckpt/file" + "{}.ckpt".format(epoch))
```

## To the cloud and back

I could *in theory* train this on my Macbook Air - perhaps with a batch size of 1 - but why torment my machine when I can outsource the job to Google. Submitting TensorFlow programs to the Cloud ML Engine is easy (and cheap) enough with the `gcloud command-line tool`. It is recommended to upload your training and validation data to Google Cloud Storage first. Then, by parametrising the trainer program, you can supply these as additional arguments when submitting the job. 

You also need to select a region, resource type, the package-path of your trainer program and the module's name, a staging bucket on GCS and the path to the tarballs of packages that you need. 
...
```{bash, eval = F}
gcloud ml-engine jobs submit training 'fer2013' --region europe-west1 \
...
I have experimented with all flavors of transfer learning for this task, and got the best result with fine tuning the last convulational block only. The worst result came from training just the classification layer. This makes sense as a dataset of cropped gray faces is too different from your regular ImageNet picture for feature extraction to be sufficient. But neither is updating all the weights necessary.
...
tab <- data.frame(Type = c("Finetuning Full Network","Fine Tuning Last Block", "Feature Extraction"),
                  Accuracy = c("93%", "93%", "29%"), Time = c("2:39h", "1:22h", "1:08h"))
kable(tab, Format = "latex") %>% row_spec(0,bold=TRUE) %>% 
kable_styling()
```
...
while(True):
    ...
    for (x,y,w,h) in faces:
        ...
        grayd = cv2.cvtColor(detected_face, cv2.COLOR_GRAY2BGR) # make it 3 channels again
