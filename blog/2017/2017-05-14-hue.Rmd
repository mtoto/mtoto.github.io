...
My original inspiration for this post was [Max Pagel's article](https://sc5.io/posts/autonomous-indoor-lighting-using-neural-networks/) on training a neural network to automatically control his Philips Hue lights. In fact, I purchased my first set of Hue bulbs because of it. 
...
Roughly 86% of the time the lamps are off, resulting in an unbalanced dataset. What about brightess values lamps were *on*, according to the three remaining categories?
...
We'll now train a [gbm](https://cran.r-project.org/web/packages/gbm/index.html) model, using the [caret](https://topepo.github.io/caret/) package, which comes with a myraid of convenience tools to make the process easier and the code a lot more concise. 
