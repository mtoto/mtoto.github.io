...
date: 2018-09-18T22:13:14-05:00
categories: ["R"]
...
3. Provisiton a MySQL database where the tweets will be written to.
...
Once the server is up and running, we can create subjects and publish messages. So time for  a simple worker program that ingests data from the Twitter Streaming API and funnels it into the database using NATS Streaming. The whole thing is barely ~50 lines of code using NATS' Go client library.
...
With a Docker image for everything nowadays, our Dockerfiles won't be long. Adding a couple of [enviromnent variables here](https://github.com/mtoto/stream-go-shiny/blob/master/db/Dockerfile) or [install additional package there](https://github.com/mtoto/stream-go-shiny/blob/master/shiny/Dockerfile), most configurations will already be taken care of by the read-only layers of the base images. 
...
- `environment: - TWITTER=${TWITTER}` ensures that the `$TWITTER` enviromnent variable (as defined in the [.env file](https://github.com/mtoto/stream-go-shiny/blob/master/.env)) is available for all.
...
After estabilishing the pool connection, it is used to check whether the latest timestamp is different from the previous one. If that's the case, we pull the last 20.000 tweets from the database, collect it as an R `data.frame` and transform it using the [tidytext](https://cran.r-project.org/web/packages/tidytext/vignettes/tidytext.html) package. 
...
With minor modifications though (and a more money to burn), we could deploy our containers onto a cluster of computers and scale the crap out of this little streaming service. Such is the beauty of cloud resources and using cloud-native technologies like Docker and NATS. 
