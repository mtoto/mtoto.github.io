...
The simplest usecase of parallelization is running the same script over and over again, but in parallel instead of in a sequential order. A classic example is simulation, ie. the random generation of numbers given a fixed set of parameters. 

I am taking an example from [Azure's tutorial on running R code in parallel](https://docs.microsoft.com/en-us/azure/batch/tutorial-r-doazureparallel#run-a-parallel-simulation), simulating stock prices after a a year (365 days) given a fixed value for standard deviation and average stock price movement per day.
...
Let's save the above script into an exacutable file, in our case `monte-carlo.R`, and write a minimal `Dockerfile` encapsulating the script. Remember kubernetes works with containers and can access them directly from [Dockerhub](https://hub.docker.com/).
...
```{bash create job}
# create 3 node cluster "kubepar" on google kubernetes engine
...
We can monitor the progress of our job using the command `kubectl get pods`, to see how many pods have succesfully run. 

![](/Users/tamas/Documents/my_site/content/blog/2018/static-pods-2.png)
...
![](/Users/tamas/Documents/my_site/content/blog/2018/stockprices.png)

# Common template and multiple parameters using expansion

Moving on, now we want to parallelize a script with different parameters at each run. Again, I am taking an example from a [doAzureParallel tutorial](http://blog.revolutionanalytics.com/2018/01/doazureparallel-simulations.html) where
...
Notice that we didn't specify parallelization parameters nor the number of completions. It's because we are going to **expand** the above template into 100 different `job.yaml` files, one for each run with a different `n` parameter for the birthday simulation. 
...
With the same command as before, we create all the jobs at once: `kubectl create -f ./jobs`. Kubernetes will automatically create, distribute and run our jobs in parallel accross pods on the nodes of our cluster.

Using the same `bash` script as before, we can retrieve the output from each run and after read it into  `R`. 

Plotting the results, the probabilty that 2 or more people will have the same birthday is 99% after 60 people in the room.
...
![](/Users/tamas/Documents/my_site/content/blog/2018/birthdays.png)
...
To illustrate the last approache, we will parallelize different regression models for the boston housing dataset, a pretty common usecase for parallelization in `R`.
...
We'll be using Redis for the work queue, so we need an additional pod running Redis and a service so other pods can retrieve the items from the work queue. The recipes for both are within [redis-master.yaml](https://github.com/mtoto/kubernetes-r-playground/blob/master/fine/redis-pod.yaml) and [redis-service.yaml](https://github.com/mtoto/kubernetes-r-playground/blob/master/fine/redis-service.yaml). Similarly to jobs, we can use `kubectl create` command to start the instances and then use the Redis command line tool to add the work items.
...
Using Google Cloud, we create a new [service account](https://cloud.google.com/compute/docs/access/service-accounts) inside our project and under **Roles** give it full access to cloud storage by selecting **Storage Object Admin**. Make sure to check the box for **Furnish a new private key** and click SAVE. 

![](/Users/tamas/Documents/my_site/content/blog/2018/service.png)
